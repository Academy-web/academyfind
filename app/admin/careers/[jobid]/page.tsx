...
export default async function JobApplicationsPage({ params }: { params: Promise<{ jobid: string }> }) {
    ...
    return (
        <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-6">
            <Link href="/admin/careers" className="inline-flex items-center text-xs font-bold text-slate-500 hover:text-purple-600 transition">
                        ...
                        {job.applications.map((app: any) => (
                            <tr key={app.id} className="hover:bg-slate-50/50 transition">
                                ...
                                <td className="p-4 text-right">
                                    <Link href={`/admin/careers/${job.id}/${app.id}`} className="inline-flex items-center gap-1.5 text-xs font-bold text-purple-600 hover:text-purple-800 bg-purple-50 hover:bg-purple-100 px-3 py-1.5 rounded-lg transition">
                                        Review Profile
                                    </Link>
                                </td>
                            </tr>
